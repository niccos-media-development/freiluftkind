...
var CustomCursor = class extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.parentElement.addEventListener(
      "pointerleave",
      __privateMethod(this, _CustomCursor_instances, onPointerLeave_fn).bind(
        this
      ),
...
var SplitLines = class extends HTMLElement {
  constructor() {
    ...
    window.addEventListener(
      "resize",
      throttle(
        __privateMethod(this, _SplitLines_instances, onWindowResized_fn).bind(
          this
        )
      )
    );
    new MutationObserver(
      __privateMethod(this, _SplitLines_instances, split_fn).bind(this, true)
    ).observe(this, {
      characterData: true,
      attributes: false,
      childList: false,
...
var EffectCarousel = class extends BaseCarousel {
  ...
  _transitionTo(
    fromSlide,
    toSlide,
    { direction = "next", animate: animate19 = true } = {}
  ) {
...
var Drawer = class extends DialogElement {
  ...
  const roundValue = window.innerWidth >= 768 ? "8px" : "0";

  switch (this.openFrom) {
    case "left":
      return document.dir === "ltr"
        ? [
            `inset(0 100% 0 0 round ${roundValue})`,
            `inset(0 0 0 0 round ${roundValue})`,
          ]
        : [
            `inset(0 0 0 100% round ${roundValue})`,
            `inset(0 0 0 0 round ${roundValue})`,
          ];
    case "right":
      return document.dir === "ltr"
    ...
    case "bottom":
      return [
        `inset(100% 0 0 0 round ${roundValue})`,
        `inset(0 0 0 0 round ${roundValue})`,
      ];
    case "top":
      return [
        `inset(0 0 100% 0 round ${roundValue})`,
        `inset(0 0 0 0 round ${roundValue})`,
      ];
    default:
      return [];
...
var ProductForm = class extends HTMLFormElement {
  ...
  async _onSubmit(event) {
    ...
    const isMainProductForm = this?.page?.value === "main-product";
    ...
    if (response.ok) {
      ...
      this.dispatchEvent(
        new CustomEvent("variant:add", {
          bubbles: true,
          detail: {
            blockCartDrawerOpening: isMainProductForm ? true : false,
...
class UpsellVariantPicker extends HTMLElement {
  constructor() {
    super();
    this.variants = this.getVariants();
    this.currentVariant = this.getCurrentVariant();
    this.optionButtons = this.querySelectorAll(
      ".upsell-product__variant-option"
    );
    this.toggleButton = this.closest(".upsell-product").querySelector(
      ".upsell-product__toggle-button"
    );
    this.selectedVariantId = this.currentVariant?.id;

    if (!this.variants || !this.currentVariant) return;
  ...
  getVariants() {
    const variantsJson = this.querySelector("[data-variants]");
    return variantsJson ? JSON.parse(variantsJson.textContent) : [];
  }

  getCurrentVariant() {
    const variantJson = this.querySelector("[data-current-variant]");
    return variantJson ? JSON.parse(variantJson.textContent) : null;
  }
  ...
  onOptionClick(event) {
    ...
    this.querySelectorAll(`[data-index="${index}"]`).forEach((btn) => {
    ...
    // Update the label text with selected value
    const optionContainer = button.closest(".upsell-product__option");
    const selectedValueSpan = optionContainer.querySelector(
      "[data-selected-value]"
    );
    ...
    const selectedOptions = Array.from(
      this.querySelectorAll('[aria-selected="true"]')
    ).map((selected) => selected.dataset.value);

    // Find new variant
    const newVariant = this.variants.find((variant) => {
      return variant.options.every(
        (option, i) => option === selectedOptions[i]
      );
    });
    ...
    if (this.toggleButton) {
      this.toggleButton.dataset.variantId = newVariant.id;

      // If product is already in cart, update the selected variant
      if (this.toggleButton.hasAttribute("data-added")) {
        const popup = this.closest('dialog[is="upsell-popup"]');
        if (popup?.selectedProducts) {
          popup.selectedProducts.delete(
            this.toggleButton.dataset.previousVariantId
          );
          popup.selectedProducts.add(newVariant.id.toString());
          this.toggleButton.dataset.previousVariantId = newVariant.id;
        }
      }
    }
  ...
  updateOptions() {
    ...
    // Set initial states
    this.currentVariant.options.forEach((optionValue, index) => {
      const button = this.querySelector(
        `[data-index="${index}"][data-value="${optionValue}"]`
      );
      if (button) {
        button.setAttribute("aria-selected", "true");
        button.classList.add("selected");
    ...
    if (this.toggleButton) {
      this.toggleButton.dataset.variantId = this.currentVariant.id;
      this.toggleButton.dataset.previousVariantId = this.currentVariant.id;
    }
  }

  updatePrice(variant) {
    const container = this.closest(".upsell-product");
...
class UpsellPopup extends HTMLDialogElement {
  constructor() {
    super();
    this.content = this.querySelector(".upsell-popup__content");
    this.overlay = this.querySelector(".upsell-popup__overlay");
    this.closeButton = this.querySelector(".upsell-popup__close");
    this.addButton = this.querySelector(".upsell-popup__add-button");
    this.skipButton = this.querySelector(".upsell-popup__skip-button");
    this.toggleButtons = this.querySelectorAll(
      ".upsell-product__toggle-button"
    );
  ...
  init() {
    // Close button event
    if (this.closeButton) {
      this.closeButton.addEventListener("click", () => this.closeUpsellPopup());
    }

    // Overlay click event
    if (this.overlay) {
      this.overlay.addEventListener("click", () => this.closeUpsellPopup());
    }

    // ESC key event
    this.addEventListener("keydown", (e) => {
      if (e.key === "Escape") this.closeUpsellPopup();
    });
    ...
    if (this.skipButton) {
      this.skipButton.addEventListener("click", () => this.closeUpsellPopup());
    }
  ...
  // Add method to properly cleanup when section is unloaded
  // disconnectedCallback() {
  //   this.closeUpsellPopup();
  ...
  //   }
  //   if (this.overlay) {
  //     this.overlay.removeEventListener("click", () => this.closeUpsellPopup());
  //   }
  //   if (this.skipButton) {
  //     this.skipButton.removeEventListener("click", () =>
  //       this.closeUpsellPopup()
  //     );
  //   }

  //   this.toggleButtons.forEach((button) => {
  //     button.removeEventListener("click", () => this.toggleProduct(button));
  //   });
  // }

  // Update show method to handle section preview
  show() {
    if (!this.open) {
      // Reset selections before showing the popup
      this.resetToggleButtons();
      this.selectedProducts.clear();
      this.updateCheckoutButton();

      // Select default variants for all variant pickers
      this.querySelectorAll("upsell-variant-picker").forEach(
        (variantPicker) => {
          const initialVariant = variantPicker.getCurrentVariant();
          if (initialVariant) {
            // Set initial states for options
            initialVariant.options.forEach((optionValue, index) => {
              const button = variantPicker.querySelector(
                `[data-index="${index}"][data-value="${optionValue}"]`
              );
              if (button) {
                button.setAttribute("aria-selected", "true");
              ...
              const selectedValueSpan = variantPicker.querySelector(
                `.upsell-product__option:nth-child(${
                  index + 1
                }) [data-selected-value]`
              );
              if (selectedValueSpan) {
      ...
      this.dispatchEvent(new CustomEvent("upsell:opened"));
    }
  }

  toggleProduct(button) {
    const isAdded = button.hasAttribute("data-added");
    const productId = button.dataset.productId;
    const variantId = button.dataset.variantId;
    const variantPicker = button
      .closest(".upsell-product")
      .querySelector(".upsell-product__options");

    if (isAdded) {
      // Remove from selection
      button.removeAttribute("data-added");
      button.querySelector(".add-text").textContent = "+ Hinzufügen";
      ...
      button.querySelector(".add-text").textContent = "✓ Hinzugefügt";
      this.selectedProducts.add(variantId);
      // Show variant picker
      if (variantPicker) {
  ...
  updateCheckoutButton() {
    if (this.addButton) {
      ...
      this.addButton.textContent =
        count > 0 ? `In den Warenkorb` : "In den Warenkorb";
      this.addButton.disabled = count === 0;
    }
  }

  // Method to get cart drawer
  getCartDrawer() {
    return document.querySelector("cart-drawer");
  }
  ...
  // Update closePopup method to use new drawer methods
  closeUpsellPopup() {
    if (this.content) {
      this.content.style.animation = "slideOut 0.2s ease-out";
    }
    if (this.overlay) {
    ...
    setTimeout(() => {
      ...
      // Open cart drawer using new method
      this.openCartDrawer();

      this.dispatchEvent(new CustomEvent("upsell:closed"));
    }, 200);
  }

  // Update addSelectedProductsToCart to use new drawer methods
  async addSelectedProductsToCart() {
    if (this.selectedProducts.size === 0) return;

    try {
      this.addButton.disabled = true;
      this.addButton.textContent = "Wird hinzugefügt...";

      // Create items array from selected products
      const items = Array.from(this.selectedProducts).map((variantId) => ({
        id: parseInt(variantId, 10),
        quantity: 1,
        properties: {
          _source: "upsell-popup",
        },
      }));
      ...
      // Use the theme's fetchCart utility
      const cartResponse = await fetch(
        window.Shopify.routes.root + "cart/add.js",
        {
          ...
          body: JSON.stringify({ items, sections: sectionsToBundle }),
        }
      );
      ...
      updatedCart["sections"] = cartJson["sections"];

      // Dispatch cart:change event with proper detail
      document.documentElement.dispatchEvent(
        new CustomEvent("cart:change", {
          bubbles: true,
          detail: {
      ...
      // Dispatch variant:add event for each added item
      items.forEach((item) => {
        this.dispatchEvent(
          new CustomEvent("variant:add", {
            bubbles: true,
      ...
      this.closeUpsellPopup(); // This will handle opening the cart drawer
    } catch (error) {
      console.error("Error adding to cart:", error);
      this.addButton.textContent = "Fehler beim Hinzufügen";
      ...
      setTimeout(() => {
        this.addButton.textContent = "In den Warenkorb";
  ...
  resetToggleButtons() {
    this.toggleButtons.forEach((button) => {
      button.removeAttribute("data-added");
      if (button.querySelector(".add-text")) {
        button.querySelector(".add-text").textContent = "+ Hinzufügen";
      }

      const upsellProduct = button.closest(".upsell-product");
      // Reset variant picker if it exists
      const variantPicker = upsellProduct.querySelector(
        "upsell-variant-picker"
      );
      if (variantPicker) {
        // Hide the variant picker
        variantPicker.style.display = "none";

        // Reset all option buttons to their initial state
        variantPicker
          .querySelectorAll(".upsell-product__variant-option")
          .forEach((btn) => {
            btn.removeAttribute("aria-selected");
            btn.classList.remove("selected");
          });

        // Reset to initial variant
        const initialVariant = JSON.parse(
          variantPicker.querySelector("[data-current-variant]").textContent
        );
        if (initialVariant) {
          // Update variant ID on toggle button
          button.dataset.variantId = initialVariant.id;
          button.dataset.previousVariantId = initialVariant.id;

          // Reset selected options to match initial variant
          initialVariant.options.forEach((optionValue, index) => {
            const optionButton = variantPicker.querySelector(
              `[data-index="${index}"][data-value="${optionValue}"]`
            );
            if (optionButton) {
  ...
  // Add method to properly handle section updates
  onSectionUpdate() {
    this.resetToggleButtons();
    this.selectedProducts.clear();
    this.updateCheckoutButton();
  ...
  // Update hasCartDrawer method to use new getter
  hasCartDrawer() {
    return !!this.getCartDrawer();
  }
}

// Update the custom element definition
if (!window.customElements.get("upsell-popup")) {
...
document.addEventListener("DOMContentLoaded", () => {
  const popup = document.querySelector('dialog[is="upsell-popup"]');
  if (popup) {
    window.upsellPopup = popup;
  }
});

if (!window.customElements.get("upsell-variant-picker")) {
  window.customElements.define("upsell-variant-picker", UpsellVariantPicker);
}
        ...
        (element) => {
          ...
          if (matchingBlock) {
            ...
            if (blockType === "buy-buttons") {
              ...
              const newSelector = matchingBlock.querySelector(
                ".quantity-selector__input"
              );
              if (newSelector) {
                newSelector.quantity = existingQuantity;
              }
            } else if (blockType === "freiluftkind-shipping") {
              element
                .querySelector(".relative")
                .replaceWith(matchingBlock.querySelector(".relative"));
            } else {
              element.replaceWith(matchingBlock);
            }
          }
...
var QuickBuyDrawer = class extends Drawer {
  ...
  _onVariantAdded(event) {
    ...
    if (this.getAttribute("no_upsell")) {
      this.shadowRoot.querySelector('[is="close-button"]').click();
    } else {
      this.replaceChildren(
        ...new DOMParser()
          .parseFromString(
    ...
    this._hasLoaded = false;
    console.log("chec", this);
    if((!this.getAttribute('id').startsWith('quick-buy-horizontal-sections')) && !window.location.pathname.startsWith('/products/')) {
      if (this.getAttribute("open_drawer")) {
        console.log(
          "checte",
        ...
        console.log("checte",this.getAttribute('id'));
    }
  }
};
...
var _VariantPicker = class _VariantPicker extends HTMLElement {
  ...
  async selectCombination({ optionValues, productChange }) {
    if(this.getAttribute('form-id').startsWith('quick-buy-form')) {
      const productPageVariantPicker = document.querySelector('.product-info__block-item > .product-info__variant-picker > variant-picker')
      if(productPageVariantPicker !== null)
        productPageVariantPicker.selectCombination({ optionValues, productChange })
    }

    const previousVariant = this.selectedVariant;
    //Faisal: Because It's creating a new deoument each time
    ...
    const newContentHtml = await __privateMethod(
      this,
      _VariantPicker_instances,
      renderForCombination_fn
    ).call(this, optionValues);
    const template = document.createElement("template");
    template.innerHTML = newContentHtml.trim();
  ...
  connectedCallback() {
    ...
    if (this.hasAttribute("show-play-button") && !this.shadowRoot) {
      this.attachShadow({ mode: "open" }).appendChild(
        document.createRange().createContextualFragment(`
        <slot></slot>
        <div class="play-button" part="play-buttonafter">
        <svg fill="none" part="play-button" width="48" height="48" viewBox="0 0 48 48">
        ...
        .play-button:after {content: "Video abspielen"; color: #1a1a1a;background: #f0f0f0;margin-top:18px;padding: 5px 10px;border-radius:20px; text-align: center; font-size: 12px; font-style: normal; font-weight: bold; line-height: normal; z-index: 1; will-change: transform; transition: transform .2s ease-in-out,opacity .2s ease-in-out,visibility .2s ease-in-out; position: absolute; top: 61%; left: 50%; transform: translate(-50%,-50%)scale(1);}
        @media screen and (max-width:699px) and (min-width:400px) {.play-button:after {top:65%;}} 
        @media screen and (max-width:399px) {.play-button:after {top:70%;}} 
        </style>
      `)
      );
    }
...
var SectionHeader = class extends HTMLElement {
  ...
  connectedCallback() {
    if (window.matchMedia("(prefers-reduced-motion: no-preference)").matches) {
      inView5(
        this,
        __privateMethod(this, _SectionHeader_instances, reveal_fn2).bind(this),
...
var StoreHeader = class extends HTMLElement {
  ...
  _onScroll() {
    ...
    if (window.scrollY < this._lastScrollTop) {
      ...
      this._accumulatedScroll > parseInt(this.getAttribute("hide-on-scroll")) &&
...
var DropdownDisclosure = class _DropdownDisclosure extends AnimatedDetails {
  ...
  _transitionIn(hasOpenSiblings = false) {
    ...
    return timeline9(timelineSequence, { delay: hasOpenSiblings > 0 ? 0.1 : 0 })
      .finished;
  }
  _transitionOut() {
    return timeline9([[this.contentElement, { opacity: 0 }, { duration: 0.2 }]])
      .finished;
  }
  /**
...
var MegaMenuDisclosure = class extends DropdownDisclosure {
   ...
   */
  _preloadImages() {
    Array.from(this.querySelectorAll('img[loading="lazy"]')).forEach((image) =>
  ...
  reinitializeDrawer() {
    ...
    Array.from(this.querySelectorAll(".panel:not(:first-child)")).forEach(
      (item) => {
        ...
        Array.from(item.querySelectorAll(".panel__wrapper")).forEach((list) =>
  ...
  async _reveal() {
    ...
    if (this.getAttribute("layout") === "stacked") {
      ...
      timeline14([
        [
          this.children,
          { opacity: 1, transform: "rotate(var(--image-rotation, 0deg))" },
          { duration: 0.3, delay: stagger6(0.1) },
        ],
      ]);
    }
...
var SlideshowCarousel = class extends EffectCarousel {
  ...
  async _transitionTo(
    fromSlide,
    toSlide,
    { direction, animate: animate19 = true } = {}
  ) {
    fromSlide.classList.remove("is-selected");
    toSlide.classList.add("is-selected");
...
// Product Info Advanced Sidebar
function initProductInfoAdvanced() {
  const sidebar = document.querySelector(".product-info-adv");
  if (!sidebar) return;

  const ctaWrapper = document.querySelector(".product-info-adv__cta-wrapper");
  ...
  function handleScroll() {
    ...
    if (isMobile) {
      if (currentScrollPosition > showOffset) {
...
document.addEventListener("DOMContentLoaded", initProductInfoAdvanced);
